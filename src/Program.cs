...
public static class Program
{
    ...
    public static int Main(string[] args)
    {
        try
        {
            // Since System.CommandLine doesn't support name-less args: Check if the first arg looks like a
            // directory and go straight to upload if it does
            ...
            Option<DirectoryInfo> validateWorkspaceOption = new("--workspace", "-w")
            {
                Description = "The directory of the workspace to validate.",
                Required = true
            };
            ...
            uploadCommand.SetAction(p =>
                UploadCommand.UploadWorkspace(p.GetRequiredValue(uploadWorkspaceOption), p.GetValue(uploadItemIdOption)));
            ...
            removeCommand.SetAction(p =>
                RemoveCommand.Remove(p.GetValue(deleteWorkspaceOption), p.GetValue(deleteItemIdOption)));

            Command validateCommand = new("validate",
                "Check a workspace and its .modinfo before uploading. Reports issues without uploading.");
