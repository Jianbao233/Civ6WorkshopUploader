...
{
    // Civ6 (Consumer) — the app the workshop items belong to.
    private static readonly AppId_t _civ6AppId = new(289070);
    ...
    public static async Task<int> UploadWorkspace(DirectoryInfo workspaceDirectory, ulong? itemIdArg)
    {
        ...
        else
        {
            ...
            SteamAPICall_t createItemCall = SteamUGC.CreateItem(_civ6AppId, EWorkshopFileType.k_EWorkshopFileTypeCommunity);
        ...
        UGCUpdateHandle_t updateHandle = SteamUGC.StartItemUpdate(_civ6AppId, workshopItem);

        // Force the primary update to write into the "english" language variant so the
        // workshop's default-fallback text is deterministic, independent of whatever
        // language the uploader's Steam client is currently set to.
        if (!SteamUGC.SetItemUpdateLanguage(updateHandle, "english"))
        {
            Log.Warn("Failed to set update language to english (default variant)!");
        }
        ...
        await UpdateLocalizations(workshopItem, modConfig.localizations ?? []);
    ...
    private static async Task UpdateLocalizations(PublishedFileId_t workshopItem, List<LocalizationConfig> localizations)
    {
        ...
        Log.Info($"Applying {localizations.Count} additional language variant(s)...");

        foreach (LocalizationConfig loc in localizations)
        {
            if (string.IsNullOrWhiteSpace(loc.language))
            {
                Log.Warn("Skipping localization entry with empty language code.");
                continue;
            }

            // english is already written by the primary update; skip to avoid an
            // unnecessary round-trip and an empty no-op SubmitItemUpdate.
            if (loc.title == null && loc.description == null && loc.changeNote == null)
            {
                Log.Warn($"Skipping localization '{loc.language}' because it has no title/description/changeNote to set.");
                continue;
            }

            Log.Info($"Updating language variant '{loc.language}'...");

            UGCUpdateHandle_t locHandle = SteamUGC.StartItemUpdate(_civ6AppId, workshopItem);

            if (!SteamUGC.SetItemUpdateLanguage(locHandle, loc.language))
            {
                Log.Warn($"Failed to set update language to '{loc.language}'!");
            }

            if (loc.title != null && !SteamUGC.SetItemTitle(locHandle, loc.title))
            {
                Log.Warn($"Failed to set localized title for '{loc.language}'!");
            }

            if (loc.description != null && !SteamUGC.SetItemDescription(locHandle, loc.description))
            {
                Log.Warn($"Failed to set localized description for '{loc.language}'!");
            }

            SteamAPICall_t call = SteamUGC.SubmitItemUpdate(locHandle, loc.changeNote ?? "");
            ...
            {
                Log.Warn($"Localization update for '{loc.language}' failed: {result.m_eResult}");
            }
            else
            {
                Log.Info($"Language variant '{loc.language}' updated.");
            }
        }
    }
